...
pub use crypto_common::{AlgorithmName, BlockSizeUser, OutputSizeUser, Reset};
...
pub type Buffer<S> = BlockBuffer<<S as BlockSizeUser>::Block, <S as BufferKindUser>::BufferKind>;
...
pub trait FixedOutputCore: UpdateCore + BufferKindUser + OutputSizeUser {
    ...
    fn finalize_fixed_core(&mut self, buffer: &mut Buffer<Self>, out: &mut Self::Output);
}
...
pub trait XofReaderCore: BlockSizeUser {
    /// Read next XOF block.
    fn read_block(&mut self) -> Self::Block;
}
